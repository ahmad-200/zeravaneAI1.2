...
def query_endpoint(request: QueryRequest):
    """
    Query ZeravaneAI with an optional live URL for context.
    If target_url is provided, scrapes (or uses cache) and answers from live docs.
    If omitted, answers from base model weights.
    """
    if not request.query.strip():
        raise HTTPException(status_code=400, detail="Query cannot be empty")

    engine = get_engine()
    answer, context_payload, scrape_method = engine.execute_live_agent_query(
        user_query=request.query,
        target_url=request.target_url.strip() if request.target_url else None,
        force_rescrape=request.force_rescrape or False,
    )

    if answer.startswith("Inference error:"):
...
def clear_cache():
    """Clear the current URL cache and ChromaDB collection."""
    engine = get_engine()
    try:
